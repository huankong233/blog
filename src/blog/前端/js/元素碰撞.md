...
那也就是说 `domA` 的 右上角 要大于 `domB` 的 左上角
...
也就是说 `domA` 的 左下角 要大于 `domB` 的 左上角
...
也就是说 `domA` 的 左上角 要小于 `domB` 的 右上角
...
也就是说 `domA` 的 左上角 要小于 `domB` 的 左下角
...
```js
function checkCollision(domA, domB) {
  return domA.offsetLeft + domA.offsetWidth > domB.offsetLeft
  // 上侧
